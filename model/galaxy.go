...
func (g *Galaxy) PredictWeather(day uint) (w Weather) {
	w.Day = day
	w.WeatherType = Unknown
	...
	if g.AreAligned(p1, p2, p3) {
		w.WeatherType = OptimalTemperature
	}

	if g.AreAlignedWithTheSun(p1, p2, p3) {
		w.WeatherType = Drought
	}
...
func (g *Galaxy) AreAligned(p3, p1, p2 Position) bool {
	a := p2.X - p1.X
	b := p2.Y - p1.Y
	c := p3.X - p1.X
	d := p3.Y - p1.Y
...
func (g *Galaxy) AreAlignedWithTheSun(p1, p2, p3 Position) bool {
	a := p2.X - p1.X
	b := p2.Y - p1.Y
	c := p3.X - p1.X
	d := p3.Y - p1.Y
...
func area(p1, p2, p3 Position) float64 {
	return math.Abs((p1.X*(p2.Y-p3.Y) + p2.X*(p3.Y-p1.Y) + p3.X*(p1.Y-p2.Y)) / 2.0)
}

func (g *Galaxy) SunInsidePlanets(p1, p2, p3 Position) (bool, Type) {
	a := area(p1, p2, p3)
...
func (g *Galaxy) Perimeter(p1, p2, p3 Position) float64 {
	d1 := p1.DistanceOf(p2)
	d2 := p2.DistanceOf(p3)
	d3 := p3.DistanceOf(p1)
...
func (g *Galaxy) isMaxPerimeter(p1, p2, p3 Position) bool {
	perimeter := g.Perimeter(p1, p2, p3)
	return perimeter >= MaxPerimeter
}

func (g *Galaxy) save(weather Weather, position Position, position2 Position, position3 Position) {
